...
var UndoManager = types.model("UndoManager", { history: types.array(types.frozen()), undoIdx: -1, targetPath: "" }).views((t2) => ({ get canUndo() {
  ...
  return { startTransaction() {
    ...
    h && this.addUndoState(), l() ? m3(t2.history[t2.undoIdx]) : t2.canUndo ? (t2.undoIdx--, i2 = true, m3(t2.history[t2.undoIdx])) : console.warn("No undo history. Please check `store.history.canUndo` before calling undo action.");
  }, redo() {
...
function starPath({ width: t2, height: h }) {
  ...
  for (let p3 = 0; p3 < 10; p3++) {
    const t3 = p3 % 2 == 0 ? a : d4, h2 = i2 + (p3 % 2 == 0 ? e5 : g2) * Math.sin(p3 * o2), L = C - t3 * Math.cos(p3 * o2);
    0 === p3 ? w2 = `M ${h2} ${L}` : w2 += ` L ${h2} ${L}`;
  }
  return w2 + " Z";
}
...
function crossPath({ width: t2, height: h }) {
  ...
  let p3 = `M ${g2},0 `;
  return p3 += `H ${d4} `, p3 += `V ${w2} `, p3 += `H ${t2} `, p3 += `V ${o2} `, p3 += `H ${d4} `, p3 += `V ${h} `, p3 += `H ${g2} `, p3 += `V ${o2} `, p3 += "H 0 ", p3 += `V ${w2} `, p3 += `H ${g2} `, p3 += "V 0 ", p3 += "Z", p3;
}
...
function figureToSvg(t2) {
  ...
  return i2 ? function(t3, h2) {
    ...
    let o2 = h2(t3), p3 = 1, L = 1;
    "string" != typeof o2 && (p3 = o2.scaleX, L = o2.scaleY, o2 = o2.path);
  ...
  <!-- Path for the fill -->
  <path d="${o2}" fill="${e5}" transform="scale(${p3}, ${L})"/>

  <!-- Path for the stroke, clipped by the star path -->
  <path d="${o2}" fill="none" stroke="${a}" stroke-width="${2 * g2}" clip-path="url(#${s})" transform="scale(${p3}, ${L})" stroke-dasharray="${w2.join(" ")}"/>
    </svg>
  `;
...
function shapeFilterToCSS(t2, e5) {
  ...
  `, n;
    ...
    case Effects.vibrance: {
      ...
      const i2 = 1 - s, o2 = i2 * r + s, f3 = i2 * l, c2 = i2 * a, m3 = i2 * r, p3 = i2 * l + s, u3 = i2 * a, h = i2 * r, w2 = i2 * l, $ = i2 * a + s;
  ...
  `, n;
    ...
    case Effects.saturation: {
      const t3 = 0.2126, r = 0.7152, l = 0.0722, a = 1 + Math.max(-1, Math.min(1, null != e5 ? e5 : 0)), s = 1 - a, i2 = s * t3 + a, o2 = s * r, f3 = s * l, c2 = s * t3, m3 = s * r + a, p3 = s * l, u3 = s * t3, h = s * r, w2 = s * l + a;
...
var d2 = async ({ element: e5, page: t2, store: n }) => {
  ...
  const f3 = g2 / l.width, b2 = u3 / l.height, y3 = g2 / u3 > e5.width / e5.height ? e5.height / u3 : e5.width / g2, w2 = g2 * y3 / f3, x = u3 * y3 / b2, $ = e5.cropX * y3 * l.width, k = e5.cropY * y3 * l.height;
  return p("div", { style: Object.assign(Object.assign({}, a), { width: "100%", height: "100%", borderRadius: e5.cornerRadius + "px", border: e5.borderSize ? `${e5.borderSize}px solid ${e5.borderColor}` : "none", backgroundRepeat: "no-repeat", backgroundImage: `url(${r})`, transform: s, backgroundSize: `${Math.round(w2)}px ${Math.round(x)}px`, backgroundPositionX: -Math.round($) + "px", backgroundPositionY: -Math.round(k) + "px" }) });
};
...
var g = { image: d2, svg: d2, text: async ({ element: e5, page: t2, store: o2 }) => {
  ...
  const f3 = u3 / s, b2 = m3 / a, y3 = n * h.width, w2 = r * h.height, x = Math.max(e5.width / h.width, e5.height / h.height), $ = { position: "absolute", width: `${Math.round(f3 * x)}px`, height: `${Math.round(b2 * x)}px`, left: -Math.round(y3 * x) + "px", top: -Math.round(w2 * x) + "px", objectFit: "fill" }, k = `video-${e5.id}`, j = p("div", { style: { position: "relative", width: "100%", height: "100%", overflow: "hidden", borderRadius: e5.cornerRadius + "px", border: e5.borderSize ? `${e5.borderSize}px solid ${e5.borderColor}` : "none" } }, p("video", { id: k, src: e5.src, style: $, controls: true, playsInline: true, muted: true, volume: e5.volume }));
...
async function u({ element: e5, page: t2, store: o2, elementHook: i2 }) {
  ...
  if (e5.blurEnabled && l.push(`blur(${e5.blurRadius / 2}px)`), e5.brightnessEnabled && l.push(`brightness(${100 * e5.brightness + 100}%)`), e5.sepiaEnabled && l.push("sepia()"), e5.grayscaleEnabled && l.push("grayscale()"), e5.filters) {
    for (const [p3, c2] of Object.entries(e5.filters)) {
      const e6 = shapeFilterToCSS(Effects[p3], c2.intensity);
...
var d3 = async ({ element: e5, page: t2, store: n }) => {
  let { src: o2 } = e5;
  if ("svg" === e5.type) {
  ...
  const g2 = await loadImage(o2), d4 = g2.width * e5.cropWidth, m3 = g2.height * e5.cropHeight, u3 = e5.width / e5.height;
  let y3, w2;
  const b2 = d4 / m3;
  "boolean" == typeof e5.stretchEnabled && e5.stretchEnabled || "svg" === e5.type ? (y3 = d4, w2 = m3) : u3 >= b2 ? (y3 = d4, w2 = d4 / u3) : (y3 = m3 * u3, w2 = m3);
  const $ = y3 / g2.width, x = w2 / g2.height, k = y3 / w2 > e5.width / e5.height ? e5.height / w2 : e5.width / y3, v = y3 * k / $, j = w2 * k / x, O = e5.cropX * g2.width * k, S = e5.cropY * g2.height * k, E = `clip-${e5.id}`, H = e5.cornerRadius || 0, M = o2.replace(/&/g, "&amp;"), R = { x: -O, y: -S, width: v, height: j, preserveAspectRatio: "none", "clip-path": `url(#${E})` }, L = [p2("clipPath", { id: E }, p2("rect", { x: 0, y: 0, width: e5.width, height: e5.height, rx: H || void 0, ry: H || void 0 }))];
  a && s && L.push(p2("mask", { id: a, maskUnits: "userSpaceOnUse", "mask-type": "alpha" }, p2("image", { href: s.replace(/&/g, "&amp;"), x: 0, y: 0, width: e5.width, height: e5.height, preserveAspectRatio: "none" }))), l && L.push(p2("mask", { id: l, maskUnits: "userSpaceOnUse", "mask-type": "alpha" }, p2("image", Object.assign({ href: M }, R))));
  let T = M;
  const P = {};
  l && h && (T = h.replace(/&/g, "&amp;"), P.mask = `url(#${l})`);
  const A = { style: { transform: c2 } };
  a && (A.mask = `url(#${a})`);
  const B = e5.borderSize || 0, C = B > 0 ? p2("rect", { x: B / 2, y: B / 2, width: Math.max(0, e5.width - B), height: Math.max(0, e5.height - B), fill: "none", stroke: e5.borderColor, "stroke-width": B, rx: Math.max(0, H - B) || void 0, ry: Math.max(0, H - B) || void 0 }) : null;
  return p2("g", A, p2("defs", {}, ...L), p2("image", Object.assign(Object.assign({ href: T }, R), P)), C);
};
var m2 = ({ element: e5, type: t2 }) => {
...
var u2 = { image: d3, svg: d3, text: async ({ element: e5, page: t2, store: n }) => {
  ...
  const f3 = "center" === e5.align ? "middle" : "right" === e5.align ? "end" : "start", d4 = s.map((t3, n2) => p2("tspan", { x: "center" === e5.align ? e5.width / 2 : "right" === e5.align ? e5.width : 0, dy: 0 === n2 ? 0 : l, innerHTML: t3 }));
  return p2("g", {}, p2("text", { fill: e5.fill, y: c2, "font-size": a + "px", "text-anchor": f3, "font-family": e5.fontFamily, "font-style": e5.fontStyle, "font-weight": e5.fontWeight, "text-decoration": e5.textDecoration, "line-height": e5.lineHeight, "letter-spacing": e5.letterSpacing + "em", "stroke-width": e5.strokeWidth, stroke: e5.stroke }, ...d4));
}, line: async ({ element: e5, page: t2, store: n }) => p2("g", {}, p2("line", { x1: 0, y1: e5.height / 2, x2: e5.width, y2: e5.height / 2, stroke: e5.color, "stroke-width": e5.height, "stroke-dasharray": e5.dash && e5.dash.length ? e5.dash.map((t3) => t3 * e5.height).join(" ") : void 0 }), p2("g", { transform: `translate(0 ${e5.height / 2})` }, m2({ element: e5, type: e5.startHead })), p2("g", { transform: `translate(${e5.width} ${e5.height / 2}) rotate(180)` }, m2({ element: e5, type: e5.endHead }))), figure: async ({ element: e5, page: t2, store: n, elementHook: r }) => {
...
async function y({ element: e5, page: t2, store: n, elementHook: r }) {
  ...
  if (e5.blurEnabled && s.push(`blur(${e5.blurRadius / 2}px)`), e5.brightnessEnabled && s.push(`brightness(${100 * e5.brightness + 100}%)`), e5.sepiaEnabled && s.push("sepia()"), e5.grayscaleEnabled && s.push("grayscale()"), e5.shadowEnabled && s.push(`drop-shadow(${e5.shadowOffsetX}px ${e5.shadowOffsetY}px ${e5.shadowBlur}px ${e5.shadowColor})`), e5.filters) {
    for (const [p3, f3] of Object.entries(e5.filters)) {
      const e6 = shapeFilterToCSS(Effects[p3], f3.intensity);
...
async function jsonToDOM2({ json: e5, elementHook: i2, fontEmbedding: l = "inline" }) {
  const h = [];
  forEveryChild({ children: e5.pages }, (e6) => {
    "text" === e6.type && -1 === h.indexOf(e6.fontFamily) && h.push(e6.fontFamily);
  });
  const c2 = "inline" === l ? await async function(e6, t2) {
  ...
  }(h, e5.fonts) : [], g2 = await Promise.all(e5.pages.map((o2) => async function({ page: e6, store: i3, elementHook: o3 }) {
  ...
  return p2("svg", { xmlns: "http://www.w3.org/2000/svg", "xmlns:xlink": "http://www.w3.org/1999/xlink", viewBox: `0 0 ${e5.width} ${e5.height}`, width: e5.width, height: e5.height }, ...c2, ...g2);
}
var b = ({ dom: t2, nestLevel: n = 0 }) => {
  if ("string" == typeof t2) {
    return t2;
  }
  if (!t2) {
  ...
  const r = t2.props, { innerHTML: i2 } = r, o2 = e2(r, ["innerHTML"]), a = Object.keys(o2).map((e5) => ((e6, t3) => "object" == typeof t3 ? `${e6}="${Object.keys(t3).map((e7) => `${e7}:${t3[e7]};`).join(" ")}"` : null == t3 || "" === t3 ? "" : `${e6}="${t3}"`)(e5, o2[e5])).filter((e5) => e5 && e5.trim().length > 0).join(" "), s = " ".repeat(n);
  return `${s}<${t2.type}${a ? " " + a : ""}>${i2 || "\n" + t2.children.map((e5) => b({ dom: e5, nestLevel: n + 1 })).join("")}${s}</${t2.type}>
`;
};
async function jsonToSVG({ json: e5, elementHook: t2, fontEmbedding: n = "inline" }) {
  const r = await jsonToDOM2({ json: e5, elementHook: t2, fontEmbedding: n });
  return b({ dom: r });
}
...
var Node = types.model("Node", { id: types.identifier, type: "none", name: "", opacity: 1, custom: types.frozen(), visible: true, selectable: true, removable: true, alwaysOnTop: false, showInExport: true }).preProcessSnapshot((e5) => {
  ...
  if (hasParentOfType(e5, GroupElement)) {
    const o2 = getParentOfType(e5, GroupElement);
    return o2 && o2.children && o2.children.length, o2;
  }
  if (hasParentOfType(e5, Page)) {
    const o2 = getParentOfType(e5, Page);
    return o2 && o2.children && o2.children.length, o2;
  }
  if (hasParentOfType(e5, Store)) {
    const o2 = getParentOfType(e5, Store);
    return o2 && o2.pages && o2.pages.length, o2;
  }
  return null;
}, get zIndex() {
  ...
  const n = JSON.parse(JSON.stringify(e5.toJSON()));
  return t2.id = t2.id || nanoid(10), forEveryChild2(n, (e6) => {
    e6.id = nanoid(10);
  }), Object.assign(n, t2), e5.page.addElement(n, { skipSelect: o2 });
}, set(t2) {
  Object.assign(e5, t2);
}, moveUp() {
...
var animations = { fade: ({ dTime: t2, element: o2, animation: i2 }) => {
  ...
  var n;
  const e5 = i2.duration, a = e5 / 2, r = null !== (n = i2.data.strength) && void 0 !== n ? n : 1, h = {}, m3 = { x: o2.x + o2.width / 3 * r, y: o2.y + o2.height / 3 * r, width: o2.width / 3 * r, height: o2.height / 3 * r, fontSize: o2.fontSize / 3 * r }, c2 = { x: o2.x, y: o2.y, width: o2.width, height: o2.height, fontSize: o2.fontSize }, d4 = t2 % e5 / a, p3 = d4 <= 1 ? d4 : 2 - d4;
  for (let s in m3) {
  ...
  var e5, a, r;
  const h = {}, m3 = n.data.direction || "right", c2 = i[m3] || i.right, d4 = null !== (e5 = n.data.movementStrength) && void 0 !== e5 ? e5 : 0.1, p3 = null !== (a = n.data.shakeStrength) && void 0 !== a ? a : 0.015;
  let s = null !== (r = n.data.zoomStrength) && void 0 !== r ? r : 0.6;
  const l = t2 / n.duration, y3 = p3 * (1 - l), f3 = Math.sin(12 * l) * y3 * 0.7 + Math.sin(8 * l) * y3 * 0.3;
  if ("exit" === n.type) {
...
var Shape = Node.named("Shape").props({ x: 0, y: 0, width: 100, height: 100, rotation: 0, opacity: 1, animations: types.array(Animation), blurEnabled: false, blurRadius: 10, brightnessEnabled: false, brightness: 0, sepiaEnabled: false, grayscaleEnabled: false, filters: types.map(ShapeFilter), shadowEnabled: false, shadowBlur: 5, shadowOffsetX: 0, shadowOffsetY: 0, shadowColor: "black", shadowOpacity: 1, visible: true, draggable: true, resizable: true, selectable: true, contentEditable: true, styleEditable: true, alwaysOnTop: false, showInExport: true }).preProcessSnapshot((t2) => {
  ...
  return { get a() {
    ...
    const p3 = t2.animations.find((t3) => "exit" === t3.type);
    if (!d4 && !c2 && (null == p3 ? void 0 : p3.enabled) && n >= t2.page.duration - p3.duration - p3.delay && n <= t2.page.duration - p3.delay) {
      const i3 = n - (t2.page.duration - p3.duration - p3.delay), o3 = animate({ element: t2, animation: p3, dTime: i3 }), a2 = getDiff(t2, o3);
      r(a2);
    }
    (null == p3 ? void 0 : p3.enabled) && n >= t2.page.duration - p3.delay && a({ opacity: 0 });
...
var TextElement = Shape.named("Text").props({ type: "text", text: "", placeholder: "", fontSize: 14, fontFamily: "Roboto", fontStyle: "normal", fontWeight: "normal", textDecoration: "", textTransform: "none", fill: "black", align: "center", width: 100, height: 0, verticalAlign: "top", strokeWidth: 0, stroke: "black", lineHeight: types.optional(types.union(types.number, types.string), 1.2), letterSpacing: 0, _editModeEnabled: false, backgroundEnabled: false, backgroundColor: "#7ED321", backgroundOpacity: 1, backgroundCornerRadius: 0.5, backgroundPadding: 0.5, curveEnabled: false, curvePower: 0.5 }).preProcessSnapshot((e5) => Object.assign({}, e5)).actions((e5) => ({ toggleEditMode(t2) {
  e5._editModeEnabled = null != t2 ? t2 : !e5._editModeEnabled, e5._editModeEnabled ? e5.store.history.startTransaction() : e5.store.history.endTransaction();
} }));

// node_modules/polotno/model/image-model.js
var ImageElement = Shape.named("Image").props({ type: "image", width: 100, height: 100, src: "", cropX: 0, cropY: 0, cropWidth: 1, cropHeight: 1, cornerRadius: 0, flipX: false, flipY: false, clipSrc: "", borderColor: "black", borderSize: 0, keepRatio: false, stretchEnabled: false }).extend((o2) => {
  const r = observable.box(false);
  let t2 = false;
  return { views: { get _cropModeEnabled() {
    return r.get();
  } }, actions: { toggleCropMode(e5) {
    const s = null != e5 ? e5 : !r.get();
    r.set(s), s && !t2 ? (o2.store.history.startTransaction(), t2 = true) : !s && t2 && (o2.store.history.endTransaction(), t2 = false);
  }, beforeDestroy() {
    t2 && (o2.store.history.endTransaction(), t2 = false);
  } } };
});
...
var Page = types.model("Page", { id: types.identifier, children: types.array(types.late(() => ElementTypes)), width: types.optional(types.union(types.number, types.literal("auto")), "auto"), height: types.optional(types.union(types.number, types.literal("auto")), "auto"), background: "white", bleed: 0, custom: types.frozen(), duration: 5e3, _exporting: false, _rendering: false, _forceMount: false }).postProcessSnapshot((e5) => {
  ...
  if (o2) {
    ...
    const r = o3.length ? getTotalClientRect(o3) : { x: 0, y: 0, width: e5.computedWidth, height: e5.computedHeight }, d4 = 1 / 4, c2 = Math.max(0, Math.min(r.x, e5.computedWidth - r.x - r.width)), m3 = e5.computedWidth * d4, h = Math.max(0, c2 - m3), l = Math.max(0, Math.min(r.y, e5.computedHeight - r.y - r.height)), p3 = e5.computedHeight * d4, g2 = Math.max(0, l - p3), u3 = e5.computedWidth - 2 * h, f3 = e5.computedHeight - 2 * g2, E = t2 / u3, x = n / f3, v = Math.min(E, x), w2 = (t2 - u3 * v) / 2 - h * v, y3 = (n - f3 * v) / 2 - g2 * v;
    forEveryChild2(e5, (e6) => {
...
var Audio = types.model("Audio", { id: types.identifier, src: "", duration: 0, startTime: 0, endTime: 1, volume: 1, delay: 0, custom: types.frozen() }).actions((t2) => ({ toJSON: () => Object.assign({}, getSnapshot(t2)) })).actions((t2) => ({ set(e5) {
  Object.assign(t2, e5);
} }));
...
var Store = types.model("Store", { role: "", pages: types.array(Page), fonts: types.array(Font), audios: types.array(Audio), width: 1080, height: 1080, currentTime: 0, isPlaying: false, scale: 1, scaleToFit: 1, unit: "px", dpi: 72, schemaVersion: 2, bleedVisible: false, rulesVisible: false, openedSidePanel: "", previousOpenedSidePanel: "", custom: types.frozen(), selectedElementsIds: types.array(types.string), animatedElementsIds: types.array(types.string), history: types.optional(UndoManager, { targetPath: "../pages" }), _elementsPixelRatio: Math.min(2, "undefined" != typeof window && window.devicePixelRatio || 1), _activePageId: "", _selectedPagesIds: types.array(types.string), _forceShowCredit: false, _key: "", _validated: false }).views((e5) => {
  ...
  return e5._selectedPagesIds.map((t2) => e5.pages.find((e6) => e6.id === t2));
}, get duration() {
  let t2 = 0;
  ...
  t2._activePageId = e5, 1 === t2._selectedPagesIds.length && (t2._selectedPagesIds = cast([e5]));
}, selectPages(e5) {
  t2._selectedPagesIds = cast(e5), 1 === t2._selectedPagesIds.length && (t2._activePageId = t2._selectedPagesIds[0]);
}, selectElements(e5) {
  ...
  const g2 = t2._elementsPixelRatio;
  d4 > t2._elementsPixelRatio && t2.setElementsPixelRatio(d4), r ? null == c2 || c2.set({ _forceMount: true }) : null == c2 || c2.set({ _exporting: true });
  const m3 = await waitTillAvailable(() => {
    const e6 = lib_default.stages.filter((e7) => e7.getAttr("pageId") === o2);
    return 0 === e6.length ? null : e6;
  });
  if (!m3) {
    throw null == c2 || c2.set({ _forceMount: false, _exporting: false }), t2.setElementsPixelRatio(g2), new Error(`Export is failed. Can not find stage for page ${o2}. Looks like <Workspace /> component is not mounted, but it is required in order to process the export.`);
  }
  m3.length > 1 && console.error(`Polotno error: Detected several canvas elements for page "${o2}" in the document. This is not supported and will lead to incorrect export. Make sure to use unique ids for all pages and make sure you unmount <Workspace /> when it is not needed.`);
  const u3 = m3[0];
  await t2.waitLoading({ _skipTimeout: s });
  const f3 = u3.findOne(".page-container");
  if (!f3) {
    throw t2.setElementsPixelRatio(g2), null == c2 || c2.set({ _forceMount: false, _exporting: false }), new Error("Export is failed. Canvas was unmounted.");
  }
  const h = u3.position();
  u3.position({ x: 0, y: 0 }), u3.find("Transformer").forEach((e6) => {
    e6.setAttr("oldVisible", e6.visible()), e6.visible(false);
  }), f3.find(".page-background").forEach((e6) => e6.shadowEnabled(false)), f3.find(".page-background").forEach((e6) => e6.strokeEnabled(false)), f3.find(".highlighter").forEach((e6) => e6.visible(false));
  const y3 = f3.findOne(".page-background-group"), b2 = y3.clip();
  y3.clip({ x: null, y: null, width: null, height: null });
  const v = f3.findOne(".elements-container"), w2 = v.clip();
  v.clip({ x: null, y: null, width: null, height: null });
  const P = f3.find((e6) => !e6.visible() && e6.getAttr("editModeEnabled") && !e6.getAttr("hideInExport"));
  P.forEach((e6) => {
    e6.setAttr("oldVisible", e6.visible()), e6.show();
  });
  const x = f3.find((e6) => e6.getAttr("hideInExport"));
  x.forEach((e6) => {
    e6.setAttr("oldVisible", e6.visible()), e6.hide();
  }), i2 && f3.find(".page-background").forEach((e6) => e6.hide());
  const _ = a ? c2.bleed : 0;
  let I = _;
  !t2.bleedVisible && a || (t2.bleedVisible || a ? t2.bleedVisible && a ? I = 0 : t2.bleedVisible && !a && (I = -c2.bleed) : I = 0);
  const k = document.createElement("canvas");
  k.width = Math.round((c2.computedWidth + 2 * _) * d4), k.height = Math.round((c2.computedHeight + 2 * _) * d4);
  const S = k.getContext("2d");
  "image/jpeg" === n && (S.fillStyle = "white", S.fillRect(0, 0, k.width, k.height));
  const O = f3.scale();
  f3.scale({ x: 1, y: 1 });
  const j = f3.toCanvas({ x: f3.x() - I, y: f3.y() - I, width: c2.computedWidth + 2 * _, height: c2.computedHeight + 2 * _, pixelRatio: d4 });
  return f3.scale(O), S.drawImage(j, 0, 0, k.width, k.height), lib_default.Util.releaseCanvas(j), i2 && f3.find(".page-background").forEach((e6) => e6.show()), x.forEach((e6) => {
    e6.visible(e6.getAttr("oldVisible"));
  }), P.forEach((e6) => {
    e6.visible(e6.getAttr("oldVisible"));
  }), f3.find(".page-background").forEach((e6) => e6.shadowEnabled(true)), f3.find(".page-background").forEach((e6) => e6.strokeEnabled(true)), u3.find("Transformer").forEach((e6) => {
    e6.visible(e6.getAttr("oldVisible"));
  }), f3.find(".highlighter").forEach((e6) => e6.visible(true)), y3.clip(b2), v.clip(w2), u3.position(h), null == c2 || c2.set({ _exporting: false, _forceMount: false }), await new Promise((e6) => setTimeout(e6)), t2.setElementsPixelRatio(g2), k;
}, async toDataURL(i2 = {}) {
  ...
  return lib_default.Util.releaseCanvas(s), r;
}, async toBlob(i2 = {}) {
  var { mimeType: o2, quality: n } = i2, a = e4(i2, ["mimeType", "quality"]);
  ...
  for (const u3 of b2) {
    const i3 = u3.map(async (i4) => {
      const o3 = e5.includeBleed ? i4.bleed : 0, n2 = i4.computedWidth + 2 * o3 + 2 * c2, r2 = i4.computedHeight + 2 * o3 + 2 * c2, l2 = d4(n2), p4 = d4(r2);
      let g3 = 0, m4 = a;
      for (; g3 < 10; ) {
        g3 += 1, 2 === g3 && console.error("Polotno can not export PDF with current settings. Quality is automatically reduced.");
        const o4 = await t2.toDataURL(Object.assign(Object.assign({}, e5), { pageId: i4.id, pixelRatio: m4 }));
        if (o4.length > 20) {
          return e5.onProgress && e5.onProgress(++v / s.length * 0.9), { url: o4, width: l2, height: p4, widthPx: n2, heightPx: r2 };
        }
        m4 *= 0.8;
      }
    });
    (await Promise.all(i3)).forEach(({ url: e6, width: t3, height: i4, widthPx: o3, heightPx: a2 }) => {
      ...
      if (s2.pageContext.cropBox = { bottomLeftX: 0, bottomLeftY: 0, topRightX: t3 * r2, topRightY: i4 * r2 }, s2.pageContext.artBox = { bottomLeftX: d4(c2 + m3) * r2, bottomLeftY: d4(c2 + m3) * r2, topRightX: d4(o3 - c2 - m3) * r2, topRightY: d4(a2 - c2 - m3) * r2 }, s2.pageContext.bleedBox = { bottomLeftX: d4(c2 + m3) * r2, bottomLeftY: d4(c2 + m3) * r2, topRightX: d4(o3 - c2 - m3) * r2, topRightY: d4(a2 - c2 - m3) * r2 }, p3) {
        y3.setLineWidth(d4(1));
        const e7 = p3 + d4(m3);
        y3.line(e7, 0, e7, p3), y3.line(0, e7, p3, e7), y3.line(t3 - e7, 0, t3 - e7, p3), y3.line(t3, e7, t3 - p3, e7), y3.line(0, i4 - e7, p3, i4 - e7), y3.line(e7, i4, e7, i4 - p3), y3.line(t3, i4 - e7, t3 - p3, i4 - e7), y3.line(t3 - e7, i4, t3 - e7, i4 - p3);
      }
      y3.addImage(e6, p3, p3, t3 - 2 * p3, i4 - 2 * p3, void 0, "FAST");
    });
  }
  return y3;
...
}, async toSVG({ elementHook: e5, pageId: i2, fontEmbedding: o2 = "inline" } = { elementHook: void 0, pageId: void 0, fontEmbedding: "inline" }) {
  var n;
  const a = t2.toJSON();
  i2 = i2 || (null === (n = a.pages[0]) || void 0 === n ? void 0 : n.id);
  const s = a.pages.find((e6) => e6.id === i2);
  return jsonToSVG({ json: Object.assign(Object.assign({}, a), { pages: s ? [s] : [] }), elementHook: e5, fontEmbedding: o2 });
}, async saveAsSVG({ fileName: e5, elementHook: i2, pageId: o2, fontEmbedding: n = "inline" } = {}) {
  const a = await t2.toSVG({ elementHook: i2, pageId: o2, fontEmbedding: n }), s = "data:text/svg;base64," + window.btoa(unescape(encodeURIComponent(a)));
  downloadFile(s, e5 || "polotno.svg");
}, async saveAsPDF(i2 = {}) {
  var { fileName: o2 } = i2, n = e4(i2, ["fileName"]);
...
//# sourceMappingURL=chunk-GDHZJU5W.js.map
