...
var TopRules = observer(({ store: r2, width: i2, height: c2 }) => {
  ...
  return import_react.default.createElement(l, { ref: u, className: "polotno-rulers" }, import_react.default.createElement(n, { style: { width: i2 + "px" }, className: "polotno-x-ruler" }, [...Array(v)].map((t3, o3) => import_react.default.createElement(s, { key: o3, style: { left: m2 + o3 * y + "px", width: y + "px" } }, "px" === r2.unit || x >= 1 ? Math.round(x * o3) : (x * o3).toFixed(1))), !!r2.selectedShapes.length && import_react.default.createElement("div", { style: { position: "absolute", left: m2 + k.x * r2.scale + "px", height: "14px", width: k.width * r2.scale, backgroundColor: "rgba(0,0,0,0.15)" } })), r2.pages.map((t3, o3) => {
    ...
    return import_react.default.createElement(a, { key: t3.id, style: { height: c2 + "px" }, className: "polotno-y-ruler" }, [...Array(w)].map((t4, o4) => import_react.default.createElement(d, { key: o4, style: { top: n2 + o4 * y - 14 + "px", width: y + "px" } }, "px" === r2.unit || x >= 1 ? Math.round(x * o4) : (x * o4).toFixed(1))), (null === (i3 = r2.selectedShapes[0]) || void 0 === i3 ? void 0 : i3.page) === t3 && import_react.default.createElement("div", { style: { position: "absolute", top: n2 + k.y * r2.scale - 14 + "px", width: "14px", height: k.height * r2.scale, backgroundColor: "rgba(0,0,0,0.15)" } }));
  }));
});
...
function handleHotkey(a3, l4) {
  var r2, d2, n2, c2;
  if ("INPUT" === (null === (r2 = document.activeElement) || void 0 === r2 ? void 0 : r2.tagName) || "TEXTAREA" === (null === (d2 = document.activeElement) || void 0 === d2 ? void 0 : d2.tagName) || "true" === (null === (n2 = document.activeElement) || void 0 === n2 ? void 0 : n2.contentEditable)) {
    return;
  }
  const s3 = l4.selectedElements.filter((e6) => e6.removable).map((e6) => e6.id);
  46 !== a3.keyCode && 8 !== a3.keyCode || l4.deleteElements(s3);
  const i2 = a3.ctrlKey || a3.metaKey, v = a3.shiftKey;
  if (!i2 || v || "z" !== a3.key.toLowerCase() && "y" !== a3.key.toLowerCase() || (a3.preventDefault(), l4.history.undo()), i2 && v && ("z" === a3.key.toLowerCase() || "y" === a3.key.toLowerCase()) && (a3.preventDefault(), l4.history.redo()), i2 && "KeyA" === a3.code) {
    a3.preventDefault();
    const e6 = null === (c2 = l4.activePage) || void 0 === c2 ? void 0 : c2.children.filter((e7) => e7.selectable), t3 = (null == e6 ? void 0 : e6.map((e7) => e7.id)) || [];
    l4.selectElements(t3);
  }
  i2 && "KeyC" === a3.code && (a3.preventDefault(), copy(l4)), i2 && "KeyX" === a3.code && (a3.preventDefault(), cut(l4)), i2 && "KeyV" === a3.code && (a3.preventDefault(), paste(l4)), "ArrowDown" === a3.code && (a3.preventDefault(), l4.selectedShapes.forEach((e6) => {
    e6.draggable && e6.set({ y: e6.y + 1 });
  })), "ArrowUp" === a3.code && (a3.preventDefault(), l4.selectedShapes.forEach((e6) => {
    e6.draggable && e6.set({ y: e6.y - 1 });
  })), "ArrowLeft" === a3.code && (a3.preventDefault(), l4.selectedShapes.forEach((e6) => {
    e6.draggable && e6.set({ x: e6.x - 1 });
  })), "ArrowRight" === a3.code && (a3.preventDefault(), l4.selectedShapes.forEach((e6) => {
    e6.draggable && e6.set({ x: e6.x + 1 });
  }));
}
...
var c = [4, 6];
var WorkspaceCanvas = observer(({ store: t3, pageControlsEnabled: a3, backgroundColor: u, pageBorderColor: d2, activePageBorderColor: h2, bleedColor: g, snapGuideStroke: p2, snapGuideStrokeWidth: f2, snapGuideDash: m2, selectionRectFill: v, selectionRectStroke: w, selectionRectStrokeWidth: b, transformLabelFill: k, transformLabelTextFill: E, components: y, onKeyDown: x, paddingX: P, paddingY: T, altCloneEnabled: R = true, visiblePagesOffset: C, renderOnlyActivePage: M }) => {
  var L;
  const S = null != P ? P : 20, F = null != T ? T : 55, [W, O] = import_react3.default.useState({ width: 100, height: 100 }), _ = import_react3.default.useRef(W), j = import_react3.default.useRef(null), z = import_react3.default.useRef(null), B = t3.bleedVisible ? Math.max(0, ...t3.pages.map((e6) => e6.bleed)) : 0, G = Math.max(...t3.pages.map((e6) => e6.computedWidth)), H = Math.max(...t3.pages.map((e6) => e6.computedHeight)), D = (null === (L = t3.activePage) || void 0 === L ? void 0 : L.computedHeight) || 0, N = G + 2 * B, I = (M ? D : H) + 2 * B, K = async ({ skipTimeout: e6 } = { skipTimeout: false }) => {
    if (e6 || await new Promise((e7) => setTimeout(e7, 50)), null === j.current) {
      return;
    }
    const r2 = j.current.getBoundingClientRect();
    0 !== r2.width && 0 !== r2.height || (console.warn("Polotno warning: <Workspace /> component can not automatically detect its size.\nWidth or height of parent elements is equal 0.\nPlease make sure it has non-zero size. You may need to adjust it with your styles. <Workspace /> will automatically fit into parent container.\nFor simpler debugging here is the log of the parent element:"), console.log(j.current));
    const n2 = z.current.clientWidth || r2.width, o3 = { width: n2, height: r2.height };
    (_.current.width !== o3.width || _.current.height !== o3.height) && (O(o3), _.current = o3);
    const i2 = (n2 - 2 * S) / N, a4 = t3.pages.length > 1 ? 3.1 : 2, l4 = (r2.height - F * a4) / I, s3 = t3.pages.length ? Math.max(Math.min(i2, l4), 0.01) : 1;
  ...
  import_react3.default.useLayoutEffect(() => {
    ...
    K();
  }, [N, I, T, P]), import_react3.default.useEffect(() => {
    t3.__();
  }, []), import_react3.default.useEffect(() => {
    const e6 = j.current;
  ...
  const Y = Math.max(S, (W.width - N * t3.scale) / 2), A = M ? 1 : t3.pages.length, V = I * t3.scale * A, X = Math.max(F, (W.height - V) / A / 2);
  import_react3.default.useEffect(() => {
    const e6 = (e7) => {
      (x || handleHotkey)(e7, t3);
    };
    ...
    return null === (e6 = z.current) || void 0 === e6 || e6.addEventListener("wheel", r2), () => {
      var e7;
      return null === (e7 = z.current) || void 0 === e7 ? void 0 : e7.removeEventListener("wheel", r2);
    };
  }, []);
  const q = import_react3.default.useRef(false);
  ((t4, r2, n2, o3, i2, a4) => {
  ...
  })(z, N * t3.scale + 2 * Y, I * t3.scale + 2 * X, t3.scale, t3, q);
  const { handleScroll: J } = ((t4, r2, n2, o3, i2, a4) => {
  ...
  })(z, I * t3.scale + 2 * X, t3, W, q, M), Q = W.width >= N * t3.scale + 2 * Y, U = u || "rgba(232, 232, 232, 0.9)", Z = t3.pages.indexOf(t3.activePage), $ = (null == y ? void 0 : y.NoPages) || l3, ee = null != C ? C : Math.min(3, Math.max(1, Math.ceil(W.height / 2 / (I * t3.scale))));
  return import_react3.default.createElement("div", { ref: j, style: { width: "100%", height: "100%", position: "relative", outline: "none", flex: 1, backgroundColor: U, overflow: "hidden" }, tabIndex: 0, className: "polotno-workspace-container" }, import_react3.default.createElement("div", { ref: z, onScroll: J, style: { position: "absolute", top: 0, left: 0, width: "100%", height: "100%", overflow: "auto", overflowX: Q ? "hidden" : "auto" }, className: "polotno-workspace-inner" }, t3.pages.map((n2, o3) => {
    const i2 = n2 === t3.activePage;
    if (M && !i2 && !n2._exportingOrRendering && !n2._forceMount) {
      return null;
    }
    if (!(Math.abs(o3 - Z) <= ee || n2._exportingOrRendering || n2._forceMount)) {
      return import_react3.default.createElement(s2, { key: n2.id, width: N * t3.scale + 2 * Y, height: I * t3.scale + 2 * X, backgroundColor: U, xPadding: Y, yPadding: X });
    }
    const l4 = import_react3.default.createElement(page_default, { key: n2.id, page: n2, xPadding: Y, yPadding: X, width: N * t3.scale + 2 * Y, height: I * t3.scale + 2 * X, store: t3, pageControlsEnabled: a3, backColor: U, pageBorderColor: d2 || "lightgrey", activePageBorderColor: h2 || "rgb(0, 161, 255)", altCloneEnabled: R, bleedColor: g || "rgba(255, 0, 0, 0.1)", selectionRectFill: v, selectionRectStroke: w, selectionRectStrokeWidth: b, snapGuideStroke: p2 || "rgb(0, 161, 255)", snapGuideStrokeWidth: f2 || 1, snapGuideDash: m2 || c, transformLabelFill: k, transformLabelTextFill: E, components: y, viewportSize: W });
    return (n2._exportingOrRendering || n2._forceMount) && !i2 && M ? import_react3.default.createElement("div", { style: { display: "none" }, key: n2.id }, l4) : l4;
  }), t3.rulesVisible && import_react3.default.createElement(TopRules, { store: t3, xPadding: Y, yPadding: X, width: N * t3.scale + 2 * Y, height: I * t3.scale + 2 * X }), 0 === t3.pages.length && import_react3.default.createElement($, { store: t3 }), t3.audios.map((r2) => import_react3.default.createElement(AudioElement, { key: r2.id, audio: r2, store: t3 }))));
});
...
var f = () => null;
var Tooltip2 = observer(({ store: t3, page: p2, components: v, stageRef: g }) => {
  var E, h2;
  const b = getTotalClientRect(t3.selectedShapes), k = t3.selectedShapes.every((e6) => e6.page === p2), x = import_react5.default.useRef(null), [y, C] = import_react5.default.useState(false), j = t3._hasCroppedImages;
  t3.selectedElements.length, import_react5.default.useEffect(() => {
    ...
    null === (e6 = null == g ? void 0 : g.current) || void 0 === e6 || e6.on("dragstart", o3), null === (t4 = null == g ? void 0 : g.current) || void 0 === t4 || t4.on("dragend", l4);
    const r2 = null === (n2 = null == g ? void 0 : g.current) || void 0 === n2 ? void 0 : n2.findOne("Transformer");
    return null == r2 || r2.on("transformstart", o3), null == r2 || r2.on("transformend", l4), () => {
      var e7, t5;
      null === (e7 = null == g ? void 0 : g.current) || void 0 === e7 || e7.off("dragstart", o3), null === (t5 = null == g ? void 0 : g.current) || void 0 === t5 || t5.off("dragend", l4), null == r2 || r2.off("transformstart", o3), null == r2 || r2.off("transformend", l4);
    };
  }, []);
  const [R, w] = import_react5.default.useState({ fit: true, needCalculate: false, token: Math.random() });
  if (import_react5.default.useEffect(() => {
    0 !== t3.selectedElements.length && w({ fit: true, needCalculate: true, token: Math.random() });
  }, [t3.selectedElements, y]), import_react5.default.useEffect(() => {
    setTimeout(() => {
      if (!x.current) {
        return;
      }
      ...
      const e6 = m(x.current, "polotno-workspace-container");
      if (!e6) {
        return;
      }
      const t4 = e6.getBoundingClientRect(), n2 = x.current.getBoundingClientRect(), o3 = (n2.right, t4.left, n2.top - t4.top), l4 = (n2.left, t4.left, n2.bottom - t4.top);
      o3 < 20 && R.fit && w({ fit: false, needCalculate: false, token: R.token }), l4 - t4.height > -20 && !R.fit && w({ fit: true, needCalculate: false, token: R.token });
    }, 10);
  }, [R.needCalculate, x.current, R.token]), import_react5.default.useEffect(() => {
    const e6 = m(g.current.content, "polotno-workspace-inner"), n2 = () => {
      t3.selectedElements.length && w({ fit: true, needCalculate: true, token: Math.random() });
    };
  ...
  const M = (null == v ? void 0 : v.Position) || PositionPicker, O = (null == v ? void 0 : v.Duplicate) || DuplicateButton, S = (null == v ? void 0 : v.Remove) || RemoveButton, T = (null == v ? void 0 : v.Group) || GroupButton, B = (null == v ? void 0 : v.Lock) || f, L = t3.selectedElements[0].type, Y = Object.assign({ TextAiWrite }, v), I = extendToolbar({ components: Y, type: L, usedItems: [] }), P = (null === (h2 = null === (E = null == g ? void 0 : g.current) || void 0 === E ? void 0 : E.findOne("Transformer")) || void 0 === h2 ? void 0 : h2.rotation()) || 0;
  let F = 30;
  return Math.abs(P) < 30 && R.fit && (F = 80), Math.abs(P) > 140 && !R.fit && (F = 80), import_react5.default.createElement(Html, { parentNodeFunc: ({ stage: e6 }) => {
    var t4, n2;
    ...
    var t4;
    const n2 = b.x + b.width / 2, o3 = R.fit ? b.y * e6.scaleY - F : b.y * e6.scaleY + b.height * e6.scaleY + F, l4 = null === (t4 = null == g ? void 0 : g.current) || void 0 === t4 ? void 0 : t4.container(), r2 = (null == l4 ? void 0 : l4.getBoundingClientRect()) || { left: 0, top: 0 }, i2 = null == l4 ? void 0 : l4.closest(".polotno-workspace-container"), s3 = (null == i2 ? void 0 : i2.getBoundingClientRect()) || { left: 0, top: 0 }, a3 = r2.left - s3.left, u = r2.top - s3.top;
    return Object.assign(Object.assign({}, e6), { x: a3 + e6.x + n2 * e6.scaleX, y: u + e6.y + o3, scaleX: 1, scaleY: 1 });
  }, divProps: { style: { pointerEvents: "none", position: "absolute", zIndex: 9 } } }, import_react5.default.createElement("div", { ref: x, style: { pointerEvents: "none" } }, import_react5.default.createElement(Navbar, { style: { padding: "2px", borderRadius: "5px", height: "34px", transform: "translate(-50%, -50%)", pointerEvents: "auto" } }, import_react5.default.createElement(NavbarGroup, { style: { height: "30px" } }, I.map((n2) => {
    const o3 = Y[n2];
    return import_react5.default.createElement(o3, { elements: t3.selectedElements, element: t3.selectedElements[0], store: t3, key: n2 });
  }), import_react5.default.createElement(T, { store: t3 }), import_react5.default.createElement(B, { store: t3 }), import_react5.default.createElement(O, { store: t3 }), import_react5.default.createElement(S, { store: t3 }), import_react5.default.createElement(M, { store: t3 })))));
});
...
//# sourceMappingURL=chunk-H7CCJQBU.js.map
